...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        if (secretKey !== process.env.ADMIN_SECRET_KEY) {
            return res.json({ success: false, message: "Secret key does not match!" });
        }
  ...
  try {
    ...
    jwt.verify(adminToken, process.env.JWT_SECRET_KEY, {}, async (err, admin) => {
      ...
      try {
        ...
        const pendingOrders = orders.filter(order => order.deliveryStatus === "Processing").length;
        ...
        res.json({
          ...
          data: {
            ...
            deliveryBoys: { active: activeDeliveryBoys, available: availableDeliveryBoys }
          }
        });
      } catch (error) {
...
router.get("/getOrdersByMonth", async (req, res) => {
  ...
  jwt.verify(adminToken, process.env.JWT_SECRET_KEY, async (err, admin) => {
      ...
      try {
          ...
          const filterQuery = filter && filter !== "All" ? { deliveryStatus: filter } : {};
...
router.get("/getAllDeliveryBoys", async (req, res) => {
  ...
  jwt.verify(adminToken, process.env.JWT_SECRET_KEY, {}, async (err, admin) => {
    ...
    try {
      const deliveryBoys = await DeliveryBoy.find({ isAvailable: true }).select("name phone");
