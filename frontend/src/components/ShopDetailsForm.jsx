...
const ShopDetailsForm = () => {
  ...
  const handleAddCategory = () => {
    if (category) {
      setShop({
        ...shop,
        productCategories: [...shop.productCategories, category],
      });
      setCategory("");
    }
  };
  ...
  return (
    <div>
      ...
      <form
        onSubmit={shopId ? editShop : addShop}
        className="bg-white dark:bg-gray-800 p-6 shadow-md rounded-lg space-y-4"
      >
        ...
        <div>
          ...
          <div className="mt-2 text-gray-600 dark:text-white">
            Current Tags:{" "}
            {shop && shop?.productCategories && shop?.productCategories.length
              ? shop?.productCategories.join(", ")
              : "No tags available"}
          </div>
        </div>
